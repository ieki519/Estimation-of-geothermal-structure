...
import numpy as np
# %%
profile_db=pd.read_csv('./input_japan/useful_data/database/profile_db_ja.csv')
geothermal_db=pd.read_csv('./input_japan/useful_data/database/geothermal_db_ja.csv')
GSJ_db=pd.read_csv('./input_japan/useful_data/database/GSJ_db_ja.csv').sort_values(["x","y"])
# %%
...
for x,y in xy_input.values:
    ...
    boring=boring[['x','y','h','z','h_z','t',"ido","keido"]]
    boring_list.append(boring)
input_data=pd.concat(boring_list).reset_index(drop=True)
input_data.to_csv('./input_japan/useful_data/input_data_ja.csv',index=False)
#%%

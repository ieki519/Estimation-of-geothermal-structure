...
for num in range(1):
    ...
    model.load_state_dict(torch.load(f'./output_japan_last/model/torch_nk500_detail_{name}_{num}.pth'))
...
est_data_input_xy.to_csv(f"./output_japan_last/voxler/nk/est_nk500_nonave_{name}_detail_input_xy.csv", index=False)
...
est_data_detail.to_csv(f'./output_japan_last/voxler/nk/est_nk500_nonave_{name}_detail.csv', index=False)

...
for except_f in ["volcano","curie",['Temp', 'pH', 'Na', 'K', 'Ca', 'Mg', 'Cl', 'SO4', 'HCO3', 'anion'],['age_a', 'age_b','age','group_ja_その他', 'group_ja_付加体', 'group_ja_堆積岩','group_ja_変成岩', 'group_ja_火成岩'],["depth0","depth500"],['grad','grad_max','grad_min','grad_max_h_z','grad_min_h_z']]:
...
# for except_f in ["depth0","depth500"]:
# for except_f in ['grad','grad_max','grad_min','grad_max_h_z','grad_min_h_z']:
    ...
    for num in range(1):
        ...
        name = "basic"
        for f in ['volcano', 'curie', 'onsen', 'tishitsu_ohe', 'depth', 'grad']:#[target_feature]:#
        ...
        train_data, test_data = extra_split80(input_data)
        ...
        if "grad" in name:
            add_input_grad = pd.read_csv(f'./input_japan/grad/add_input_grad_ja_extra80_{num}.csv')#80
            add_grid_grad = pd.read_csv(f'./input_japan/grad/add_grid_grad_detail_ja_extra80_{num}.csv')#80
        ...
        model.load_state_dict(torch.load(f'./output_japan_last/model/torch_dnn_extra80_{name}_{num}.pth'))
        model.eval()
...
e0=pd.read_csv(f'./output_japan_last/learning_curve/select_features/lc_dnn_extra80_{name}_0.csv')["test_loss"].values[-1]
ep["PI"]=ep["test_loss"]/e0
ep.sort_values("PI",ascending=False).to_csv(f"./output_japan_last/PI/{name}_PI_extra80.csv",index=False)
# %%

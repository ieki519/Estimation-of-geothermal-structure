...
train_data_d[train_data_d.h_z<=-1000]
#%%
767,546,246
...
xy.shape
#%%
sns.set("talk","whitegrid",font='Yu Gothic')
# sns.set_context()
fig,ax = plt.subplots()
...
fig.savefig(f"./修論用画像/パワポ/地温と標高の散布図.png",dpi=300,bbox_inches = 'tight')
...
#%%
sns.set(font='Yu Gothic')
a=sns.jointplot(x="t",y="h_z",data=input_data)
...
ax.legend(["BASEモデル","火山モデル"],prop={'family':"MS Gothic"})#,bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0)
...
onsen=onsen.groupby(["x","y"],as_index=False).mean()
onsen=onsen[["地温",'泉温', 'pH', 'Na', 'K', 'Ca', 'Mg', 'Cl', "SO$_{4}$", "HCO$_{3}$", 'AI']]
cmap = sns.color_palette("seismic", 200)
grid_kws = {"height_ratios": (.9, .1), "hspace": 1}
fig,(ax1,ax2) =plt.subplots(2,gridspec_kw=grid_kws)
sns.heatmap(onsen.corr(), ax=ax1,cbar_ax=ax2,vmax=1, vmin=-1,square=True, cmap=cmap,cbar_kws={"orientation": "horizontal"})
plt.savefig(f"./修論用画像/パワポ/温泉相関係数.png",dpi=300,bbox_inches='tight')
# %%
#%%
# learning inter
fig, ax = plt.subplots()
...
for key in ['Temp', 'pH', 'Na', 'K', 'Ca', 'Mg', 'Cl', 'SO4', 'HCO3','anion']:
    ...
    # ax.grid()
    fig.savefig(f"./修論用画像/パワポ/onsen/{key}_est.png",dpi=300,bbox_inches='tight')
    plt.show()
...
# ax.plot(lc['epoch'],lc['test_loss'])
ax.legend(["BASEモデル","温泉モデル"],prop={'family':"MS Gothic"})#,bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0)
fig.savefig(f"./修論用画像/パワポ/温泉_extra.png",dpi=300,bbox_inches = 'tight')
plt.show()
# %%
...
fig.savefig(f"./修論用画像/パワポ/地質ohe_inter.png",dpi=300,bbox_inches='tight')
plt.show()
#%%
# learning extra
fig, ax = plt.subplots()
...
# ax.plot(lc['epoch'],lc['test_loss'])
ax.legend(["BASEモデル","地質モデル"],prop={'family':"MS Gothic"})
fig.savefig(f"./修論用画像/パワポ/地質ohe_extra.png",dpi=300,bbox_inches='tight')
plt.show()
#%%
# %%
...
fig.savefig(f"./修論用画像/パワポ/標高別地温_inter.png",dpi=300,bbox_inches='tight')
plt.show()
#%%
# learning extra
fig, ax = plt.subplots()
...
# ax.plot(lc['epoch'],lc['test_loss'])
ax.legend(["BASEモデル","標高別地温モデル"],prop={'family':"MS Gothic"})
fig.savefig(f"./修論用画像/パワポ/標高別地温_extra.png",dpi=300,bbox_inches='tight')
plt.show()
# %%
...
# ax.set_ylim(25,40)
# ax.set_title("内挿評価")
color ={"":"black","_grad_only":"blue","_grad":"red"}
diff_list=[]
...
fig.savefig(f"./修論用画像/パワポ/地温勾配_inter.png",dpi=300,bbox_inches='tight')
plt.show()
#%%
# learning extra
fig, ax = plt.subplots()
...
# ax.plot(lc['epoch'],lc['test_loss'])
ax.legend(["BASEモデル","地温勾配モデル①","地温勾配モデル②"],prop={'family':"MS Gothic"})
fig.savefig(f"./修論用画像/パワポ/地温勾配_extra.png",dpi=300,bbox_inches='tight')
plt.show()
# %%
# %%
...
#     lc=pd.read_csv(f'./output_japan_last/learning_curve/nk/lc_nk_extra_{i}_0.csv')[::10]
#     ax.plot(lc['epoch'],lc['test_loss'],c="red")
fig.savefig(f"./修論用画像/パワポ/ALL_inter.png",dpi=300,bbox_inches='tight')
...
#     lc=pd.read_csv(f'./output_japan_last/learning_curve/nk/lc_nk_extra_{i}_0.csv')[::10]
#     ax.plot(lc['epoch'],lc['test_loss'],c="red")
fig.savefig(f"./修論用画像/パワポ/ALL_extra.png",dpi=300,bbox_inches='tight')
...
ax.legend(["Deep Neural Network","Neural Kriging"])#,"SELECTモデル(DNN)","SELECTモデル(NK)"],ncol = 2,bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0)
print(diff_list[0]-diff_list[-1])
# ax.legend(["BASEモデル","活火山モデル","キュリー点深度モデル","地質モデル","温泉モデル","標高別地温モデル","地温勾配モデル","ALLモデル"],ncol = 4,bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0)
# for i in ["basic"]:#"basic_volcano_curie_onsen_depth800_grad"]:#'basic_onsen_tishitsu_ohe_depth800_grad']:
#     lc=pd.read_csv(f'./output_japan_last/learning_curve/nk/lc_nk_extra_{i}_0.csv')[::10]
#     ax.plot(lc['epoch'],lc['test_loss'],c="red")
fig.savefig(f"./修論用画像/パワポ/NK_inter2.png",dpi=300,bbox_inches='tight')
...
fig.savefig(f"./修論用画像/パワポ/NK_extra2.png",dpi=300,bbox_inches='tight')

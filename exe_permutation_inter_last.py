...
# name = "basic"
# for f in all_features_name_list[-1]:
#     name += "_"+f 
# all_features_name_list[-1]
#%%
permutation_dict={}
# target_feature="grad"
#%%
# for except_f in ["volcano","curie",['Temp', 'pH', 'Na', 'K', 'Ca', 'Mg', 'Cl', 'SO4', 'HCO3', 'anion'],['age_a', 'age_b','age',"group_rank"],["depth0","depth500","depth1000"],['grad','grad_max','grad_min','grad_max_h_z','grad_min_h_z']]:
...
# for except_f in ["depth0","depth500","depth1000"]:
# for except_f in ['grad','grad_max','grad_min','grad_max_h_z','grad_min_h_z']:
    ...
    for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
        ...
        model.load_state_dict(torch.load(f'./output_japan_last/model/torch_dnn_inter_{name}_{num}.pth'))
        model.eval()
...
e0=pd.read_csv(f'./output_japan_last/learning_curve/select_features/lc_dnn_inter_{name}.csv')["test_loss"].values[-1]
ep["PI"]=ep["test_loss"]/e0
ep.sort_values("PI",ascending=False).to_csv(f"./output_japan_last/PI/{name}_PI_inter.csv",index=False)
# %%

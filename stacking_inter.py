...
for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
    ...
    pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
        f'./output/learning_curve/lc_dnn_inter_stacking_{num}.csv', index=False)
    torch.save(model.state_dict(), f'./output/model/torch_dnn_inter_stacking_{num}.pth')
    print('-'*50)
...
est_data.to_csv(f'./output/voxler/est_dnn_inter_stacking.csv', index=False)
...
pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output/learning_curve/lc_dnn_inter_stacking.csv', index=False)

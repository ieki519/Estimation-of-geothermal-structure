...
#%%
for unit_num in [10]:#[30,60,90,120,150,180]:
    ...
    for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
        ...
        loss_list = []
        learning_curve_list=[]
        model = Net(X_train.shape[1], 1, unit_num).to(device)
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
            f'./output_japan/learning_curve/lc_dnn_inter_{name}_{num}.csv', index=False)
    ...
    pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output_japan/learning_curve/lc_dnn_inter_{name}.csv', index=False)

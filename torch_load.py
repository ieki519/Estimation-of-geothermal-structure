...
def preprocess_grid_detail(df):
    add_grid_volcano_detail = pd.read_csv('./input/volcano/add_grid_volcano_detail.csv')
    add_grid_curie_detail = pd.read_csv('./input/curie_point/add_grid_curie_detail.csv')
    add_grid_tishitsu_detail = pd.read_csv('./input/tishitsu/add_grid_tishitsu_detail_pred.csv')
    add_grid_onsen_detail = pd.read_csv('./input/onsen/add_grid_onsen_detail.csv')
    # add_grid_kmeans = pd.read_csv('./input/k_means/add_grid_kmeans.csv')

    df = df.merge(add_grid_volcano_detail, how='left', on=['x', 'y'])
    df = df.merge(add_grid_curie_detail, how='left', on=['x', 'y'])
    df = df.merge(add_grid_tishitsu_detail, how='left', on=['x', 'y'])
    df = df.merge(add_grid_onsen_detail, how='left', on=['x', 'y'])
    # df = df.merge(add_grid_kmeans, how='left', on=['x', 'y'])
...
for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
    if num!=5:
        ...
        est_data_detail = pd.read_csv('./input/useful_data/est_grid_detail.csv')
        ...
        est_data_detail = preprocess_grid_detail(est_data_detail)
        ...
        model.train()
        model.load_state_dict(torch.load(f'./output/model/torch_nk_inter_addz_new_{num}.pth'))
        ...
        #     f'./output/learning_curve/lc_nk_inter_{num}.csv', index=False)
        print('-'*50)
        est_data_detail['t'] = convert_data(X_est_detail).detach().cpu().numpy()
        est_data_detail = est_data_detail[['x', 'y', 'h_z', 't']]
        est_data_detail.to_csv(f'./output/voxler/est_nk_inter_addz_new_{num}_detail.csv', index=False)
        # torch.save(model.state_dict(), f'./output/model/torch_nk_inter_{num}.pth')

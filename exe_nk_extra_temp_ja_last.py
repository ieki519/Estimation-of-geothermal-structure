...
for num in range(5):
    if num==args.number:
        ...
        est_data.to_csv(f'./output_japan_last/voxler/est_nk{nk_hyper}_extra_{name}_{num}.csv', index=False)
        torch.save(model.state_dict(), f'./output_japan_last/model/torch_nk{nk_hyper}_extra_{name}_{num}.pth')

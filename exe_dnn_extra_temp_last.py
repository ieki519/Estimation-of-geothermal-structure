...
for all_f in [["curie","onsen","tishitsu_ohe","depth","grad"]]:
    ...
    for num in range(1):
        ...
        if "grad" in name:
            add_input_grad = pd.read_csv(f'./input_japan/grad/add_input_grad_ja_extra_{num}.csv')
            add_grid_grad = pd.read_csv(f'./input_japan/grad/add_grid_grad_detail_ja_extra_{num}.csv')
        ...
        test_data = test_data[test_data.h_z==-1500]
        ...
        #     f'./output_japan_last/learning_curve/select_features/lc_dnn_extra_{name}_{num}.csv', index=False)
        # torch.save(model.state_dict(), f'./output_japan_last/model/torch_dnn_extra_{name}_{num}.pth')

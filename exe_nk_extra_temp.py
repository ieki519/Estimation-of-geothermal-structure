...
for num in range(5):
    if num==args.number:
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss','sv_loss']).to_csv(
            f'./output/learning_curve/lc_nk_extra_addz_new_epoch2000{num}.csv', index=False)
        print('-'*50)
        est_data['t'] = convert_data(X_est).detach().cpu().numpy()
        est_data = est_data[['x', 'y', 'h_z', 't']]
        est_data.to_csv(f'./output/voxler/est_nk_extra_addz_new_epoch2000{num}.csv', index=False)
        torch.save(model.state_dict(), f'./output/model/torch_nk_extra_addz_new_epoch2000{num}.pth')

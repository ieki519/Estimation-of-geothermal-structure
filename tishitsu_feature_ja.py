...
import folium
#%%
add_input_tishitsu_idokeido=pd.read_csv("./input_japan/tishitsu/database/add_input_tishitsu_idokeido.csv")
add_grid_tishitsu_idokeido =pd.read_csv("./input_japan/tishitsu/database/add_grid_tishitsu_detail_idokeido_ja.csv")
add_input_tishitsu_idokeido.ido,add_input_tishitsu_idokeido.keido = add_input_tishitsu_idokeido.ido.round(8),add_input_tishitsu_idokeido.keido.round(8)
add_grid_tishitsu_idokeido.ido,add_grid_tishitsu_idokeido.keido = add_grid_tishitsu_idokeido.ido.round(8),add_grid_tishitsu_idokeido.keido.round(8)
add_grid_tishitsu_idokeido
#%%
...
# m
#%%
input_data=pd.read_csv('./input_japan/useful_data/input_data_ja.csv').groupby(['x','y'],as_index=False).mean()[['x','y','h',"ido","keido"]]
est_grid=pd.read_csv('./input_japan/useful_data/est_grid_detail_ja.csv').groupby(['x','y'],as_index=False).mean()[['x','y','h']]
wgs_albers_grid = pd.read_csv("./input_japan/WGS_to_albers_ja/grid_WGS_albers_detail_ja.csv")[["x","y","ido","keido"]]
est_grid=est_grid.merge(wgs_albers_grid,how="left",on=["x","y"])
#%%
input_data.ido,input_data.keido = input_data.ido.round(8),input_data.keido.round(8)
est_grid.ido,est_grid.keido = est_grid.ido.round(8),est_grid.keido.round(8)
#%%
add_input_tishitsu=input_data.merge(add_input_tishitsu_idokeido,how='left',on=['ido','keido'])
add_grid_tishitsu=est_grid.merge(add_grid_tishitsu_idokeido,how='left',on=['ido','keido'])
#%%
...
add_input_tishitsu.to_csv('./input_japan/tishitsu/add_input_tishitsu_pred_ja.csv',index=False)
add_grid_tishitsu.to_csv('./input_japan/tishitsu/add_grid_tishitsu_detail_pred_ja.csv',index=False)
...
tishitsu=pd.concat([add_input_tishitsu,add_grid_tishitsu])
tishitsu_age = pd.DataFrame(np.sort(tishitsu['formationAge_ja'].unique()))#.to_csv('./input_japan/成形前データ/地質年代_ja.csv',index=False,encoding='cp932')
tishitsu_age
#%%
import codecs
with codecs.open(rf'./input_japan/tishitsu/地質年代.csv','r','shift-jis','ignore') as f:
    age=pd.read_csv(f)
age
#%%
# %%
for i in tishitsu_age.values:
    if age.loc[age["formationAge_ja"]==i[0],:].size==0:
        print(i[0])
# %%

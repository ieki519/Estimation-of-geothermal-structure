...
for name in master_features_dict.keys():
    ...
    for num in range(5):
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
            f'./output/learning_curve/lc_dnn_extra_{name}_{num}.csv', index=False)
        torch.save(model.state_dict(), f'./output/model/torch_dnn_extra_{name}_{num}.pth')
        print('-'*50)
    pred_est = np.hstack(pred_est_list).mean(axis=1)
    est_data['t'] = pred_est
    est_data = est_data[['x', 'y', 'h_z', 't']]
    est_data.to_csv(f'./output/voxler/est_dnn_extra_{name}.csv', index=False)
    ...
    pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output/learning_curve/lc_dnn_extra_{name}.csv', index=False)

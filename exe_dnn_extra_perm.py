...
#                     }
#%%
# all_features_name_list = ["volcano","curie","onsen","tishitsu","depth","grad"]
all_features_name_list = ["grad"]

features_dict = {
                "volcano":['volcano'],
                "curie":['curie'],
                "onsen":['Temp', 'pH', 'Na', 'K', 'Ca', 'Mg', 'Cl', 'SO4', 'HCO3', 'anion'],
                "tishitsu_ohe":['age','group_ja_その他', 'group_ja_付加体', 'group_ja_堆積岩','group_ja_変成岩', 'group_ja_火成岩'],
                "tishitsu_rank":["nan",'age','group_rank'],
                "depth":["depth0","depth500","depth800"],#extraは800
...
for all_f in all_features_name_list:
    ...
    for except_f in features_dict[all_f]:
        for num in range(1):
            ...
            if "nan" in master_features:
                master_features.remove("nan")
            
            cat_features = ['group_ja_その他', 'group_ja_付加体', 'group_ja_堆積岩','group_ja_変成岩', 'group_ja_火成岩']
            except_name = "except_"+except_f
            except_features = [except_f]
            print(name+"_"+except_name)
            ...
            pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
                f'./output_japan/learning_curve/perm/lc_dnn_extra_{name}_{except_name}_{num}.csv', index=False)
            torch.save(model.state_dict(), f'./output_japan/model/torch_dnn_extra_{name}_{except_name}_{num}.pth')
            print('-'*50)
    # pred_est = np.hstack(pred_est_list).mean(axis=1)
    # est_data['t'] = pred_est
    # est_data = est_data[['x', 'y', 'h_z', 't']]
    # est_data.to_csv(f'./output_japan/voxler/est_dnn_extra_{name}_{except_name}.csv', index=False)
    ...
    # pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output_japan/learning_curve/lc_dnn_extra_{name}_{except_name}.csv', index=False)

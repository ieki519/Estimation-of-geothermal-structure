    ...
    if num==args.number:
        ...
        for epoch in range(1001):
            ...
            if epoch < 0:
                ...
                loss_sum = loss+Min_t(pred_est, 0)+(1/10000)*loss_curie+Max_t_580(pred_curie_580ika)+(1/10000)*Min_t_580(pred_curie_580izyou)+Max_t(pred_curie_580izyou,800)+(1/1000)*loss_sv
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss','sv_loss']).to_csv(
            f'./output_japan/learning_curve/lc_nk_output_{name}_{num}.csv', index=False)
        print('-'*50)
        ...
        torch.save(model.state_dict(), f'./output_japan/model/torch_nk_output_{name}_{num}.pth')

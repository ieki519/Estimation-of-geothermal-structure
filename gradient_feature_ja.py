...
features =['x', 'y', 'h', 'volcano', 'curie',
       ...
       'group_ja_変成岩', 'group_ja_火成岩', 'age',"depth0","depth500","depth1000"]#が入っていない
result_df=[]
...
result_df
#%%
result_df.to_csv("./input_japan/grad/database/grad_pred_result.csv")
#%%
def variogram(df, sep=11000, max_dist=500001, parameters={'sill': 9, 'range': 200000, 'nugget': 0.5}, how='gaussian'):
    ...
    plt.show()
#%%
variogram(xy_input_data[["x","y","grad"]], sep=1100, max_dist=50001, parameters={
...
grad_features = ['x', 'y', 'h', 'volcano', 'curie',
       'age_a', 'age_b', 'Temp', 'pH', 'Na', 'K', 'Ca', 'Mg', 'Cl', 'SO4',
       'HCO3', 'anion', 'group_ja_その他', 'group_ja_付加体', 'group_ja_堆積岩',
       'group_ja_変成岩', 'group_ja_火成岩', 'age',"depth0","depth500","depth1000"]
#%%
# all
input_data = pd.read_csv('./input_japan/useful_data/input_data_ja.csv')
input_data = preprocess_input(input_data)
...
for grad_target in ['grad','grad_min','grad_max_h_z','grad_min_h_z']:
    X_train = input_data_plane[grad_features+[grad_target]]
    ...
    est_data_plane[grad_target] = Stacking_model(X_train,grad_target,X_est)
for grad_target in['grad_max']:
    X_train = input_data_plane[grad_features]
    Y_train = input_data_plane[grad_target]
    X_est = est_data_plane[grad_features]
    rf = RandomForestRegressor(random_state=0)
    rf.fit(X_train,Y_train)
    est_data_plane[grad_target] = rf.predict(X_est)
input_data_plane=input_data_plane[["x","y"]+grad_target_list]
est_data_plane=est_data_plane[["x","y"]+grad_target_list]
input_data_plane.to_csv("./input_japan/grad/add_input_grad_detail_ja.csv",index=False)
# est_data_plane.to_csv("./input_japan/grad/add_grid_grad_detail_ja.csv",index=False)
#%%
...
for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
    ...
    train_data=train_data.merge(add_input_depth,on=["x","y"],how="left")
    test_data=test_data.merge(add_input_depth,on=["x","y"],how="left")
    est_data=est_data.merge(add_grid_depth,on=["x","y"],how="left")
    ...
    for grad_target in ['grad','grad_min','grad_max_h_z','grad_min_h_z']:
        ...
        # est_data_plane[grad_target] = Stacking_est(stacking_model,X_train,grad_target,X_est)
        test_data_plane[grad_target] = Stacking_model(X_train,grad_target,X_test)
        est_data_plane[grad_target] = Stacking_model(X_train,grad_target,X_est)
    for grad_target in ['grad_max']:
        ...
        rf = RandomForestRegressor(random_state=0)
        rf.fit(X_train,Y_train)
        test_data_plane[grad_target] = rf.predict(X_test)
        est_data_plane[grad_target] = rf.predict(X_est)
        
    df_input = pd.concat([train_data_plane,test_data_plane])
    df_input=df_input[["x","y"]+grad_target_list]
    # df_input.to_csv(f"./input_japan/grad/add_input_grad_ja_inter_{num}.csv",index=False)
    est_data_plane = est_data_plane[["x","y"]+grad_target_list]
    # est_data_plane.to_csv(f"./input_japan/grad/add_grid_grad_detail_ja_inter_{num}.csv",index=False)
...
grad_features = ['x', 'y', 'h', 'volcano', 'curie',
       'age_a', 'age_b', 'Temp', 'pH', 'Na', 'K', 'Ca', 'Mg', 'Cl', 'SO4',
       'HCO3', 'anion', 'group_ja_その他', 'group_ja_付加体', 'group_ja_堆積岩',
       'group_ja_変成岩', 'group_ja_火成岩', 'age',"depth0","depth500"]#,"depth800"
fix_seed(0)
input_data = pd.read_csv('./input_japan/useful_data/input_data_ja.csv')
input_data = preprocess_input(input_data)
...
add_input_depth = pd.read_csv(f"./input_japan/depth/add_input_depth_ja_extra_0.csv")
add_grid_depth = pd.read_csv(f"./input_japan/depth/add_grid_depth_detail_ja_extra_0.csv")
...
df_input = pd.concat([train_data_plane,test_data_plane])
df_input=df_input[["x","y"]+grad_target_list].groupby(["x","y"],as_index=False).mean()
est_data_plane=est_data_plane[["x","y"]+grad_target_list]
# df_input.to_csv(f"./input_japan/grad/add_input_grad_ja_extra_0.csv",index=False)
# est_data_plane.to_csv(f"./input_japan/grad/add_grid_grad_detail_ja_extra_0.csv")
#%%
print("end")
#%%
est_data_plane
...
x=range(0,max_dist,sep)
plt.plot(x[:],sv_i[:],c='black',marker='o')
plt.plot(x[:],vm.spherical_variogram_model([4000-1000,500,1000],np.array(x))[:],c='red')
...
for trn_idx, val_idx in cv.split(trn, tst):
    ...
    ok = OrdinaryKriging(trn_x['x'], trn_x['y'], trn_y,variogram_model='spherical',variogram_parameters={'sill':4000-1000,'range':500,'nugget':1000}) 

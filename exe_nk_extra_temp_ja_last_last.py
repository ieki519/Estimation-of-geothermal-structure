...
def calc_model_feature_semivariogram(df_input,df_est):
    ...
    sv_feature = ["x","y","volcano", "curie", 'age', 'group_ja_堆積岩', 'group_ja_火成岩', "Temp", "Na",
                  "Cl", "grad", "grad_max", "grad_max_h_z", "depth0", "depth500"]
    df_input = df_input.copy()
    df_est = df_est.copy()
    ...
    # calc model sv
    sv_all = []
    for h_z in [-750,-500,-250,0,250,500]:
        # preprocess
...
features_dict = {
                ...
                "depth":["depth0","depth500"],#extraは800
...
for num in range(5):
    if num==args.number:
        ...
        # all_f = ["volcano","curie","onsen","depth800","grad"]
        # all_f = ["onsen","tishitsu_ohe","depth800","grad"]
        ...
        est_data = pd.read_csv('./input_japan/useful_data/est_grid_sv250_ja.csv').sort_values(["x","y","h_z"])
        ...
        # depth feature 
        if "depth" in name:
            add_input_depth = pd.read_csv(f'./input_japan/depth/add_input_depth_ja_extra_{num}.csv')
            add_grid_depth = pd.read_csv(f'./input_japan/depth/add_grid_depth_detail_ja_extra_{num}.csv')
        ...
        # grad_feature
        if "grad" in name:
            add_input_grad = pd.read_csv(f'./input_japan/grad/add_input_grad_ja_extra_{num}.csv')#80
            add_grid_grad = pd.read_csv(f'./input_japan/grad/add_grid_grad_detail_ja_extra_{num}.csv')#80
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss','sv_loss']).to_csv(
            f'./output_japan_last/learning_curve/nk/lc_nk_svf_{nk_hyper_f}_{nk_hyper_z}_extra_{name}_{num}.csv', index=False)
        print('-'*50)
        est_data['t'] = convert_data(X_est).detach().cpu().numpy()
        est_data = est_data[['x', 'y', 'h_z', 't']]
        est_data.to_csv(f'./output_japan_last/voxler/est_nk_svf_{nk_hyper_f}_{nk_hyper_z}_extra_{name}_{num}.csv', index=False)
        torch.save(model.state_dict(), f'./output_japan_last/model/torch_nk_svf_{nk_hyper_f}_{nk_hyper_z}_extra_{name}_{num}.pth')

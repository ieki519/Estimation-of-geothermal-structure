...
def calc_model_feature_semivariogram(df_input,df_est):
    # preprocess
    sv_feature = ["x","y","volcano", "curie", 'age', 'group_ja_堆積岩', 'group_ja_火成岩', "Temp", "Na",
    ...
    # calc model sv
    sv_all = []
    for h_z in [-1000,-750,-500,-250,0,250,500]:
        # preprocess
...
def calc_pred_semivariogram_z(ts_est,ts_est_xy,dist_z_list):
    ...
    for i,xy in enumerate(ts_est_xy):
        ...
        dist_t=pairwise_distances(pred_t)
        sv_i=[]
        for z in range(250,1001,250):#1501
...
for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
    if num==args.number:
        ...
        # all_f = ["volcano","curie","onsen","depth","grad"]
        # all_f = ["onsen","tishitsu_ohe","depth","grad"]
        # all_f = ["volcano","curie","onsen","tishitsu_rank","depth","grad"]
        # all_f = ["curie","onsen","tishitsu_ohe","depth","grad"]
        ...
        # preprocess 
        est_data = pd.read_csv('./input_japan/useful_data/est_grid_sv250_ja.csv')
        ...
        # depth feature 
        if "depth" in name:
            add_input_depth = pd.read_csv(f'./input_japan/depth/add_input_depth_ja_inter_{num}.csv')
            add_grid_depth = pd.read_csv(f'./input_japan/depth/add_grid_depth_detail_ja_inter_{num}.csv')
            
            train_data = train_data.merge(add_input_depth, how='left', on=['x', 'y'])
        ...
        if "grad" in name:
            ...
            add_grid_grad = pd.read_csv(f'./input_japan/grad/add_grid_grad_detail_ja_inter_{num}.csv')
            
            train_data = train_data.merge(add_input_grad, how='left', on=['x', 'y'])
        ...
        # test_data_round = test_data.copy()
        # test_data_round["h_z"] = test_data_round["h_z"].round(-1)
        # test_data_round = test_data_round.groupby(["x","y","h_z"],as_index=False).mean()
        # test_data_round_origin = test_data_round[['x', 'y', 'h_z']].values
        # test_data_round_origin_xy = test_data_round[['x', 'y', 'h_z']].groupby(["x","y"],as_index=False).mean()[["x","y"]].values
        ...
        # test_data_round_origin = Variable(torch.from_numpy(
        #     test_data_round_origin).float()).to(device)
        # test_data_round_origin_xy = Variable(torch.from_numpy(
        #     test_data_round_origin_xy).float()).to(device)
        ...
        # feature
        model_sv_f,dist_f_list = calc_model_feature_semivariogram(train_data,est_data)
        print(model_sv_f)
        
        # z
        # model_sv_z=calc_model_semivariogram_z(train_data,est_data)
        dist_z_list = create_d_z_list(est_data_origin,est_data_origin_xy)
        # dist_z_list_test = create_d_z_list(test_data_round_origin,test_data_round_origin_xy)
        ...
        sv_target_list = ["sv_250","sv_500","sv_750","sv_1000"]
        for sv_target in sv_target_list:
            ...
            train_data_plane_isnull[sv_target] = Stacking_model(X_train,sv_target,X_train_isnull)
            test_data_plane[sv_target] = Stacking_model(X_train,sv_target,X_test)
            ...
            # test_data_round = test_data_round.merge(test_data_plane[['x', 'y', sv_target]], how='left', on=['x', 'y'])
        ...
        # model_sv_z_test = test_data_round.groupby(["x","y"],as_index=False).mean()[sv_target_list].values
        # model_sv_z_test = Variable(torch.from_numpy(model_sv_z_test).float()).to(device)
        
        # model_sv_z = torch.cat((model_sv_z,model_sv_z_test),0)
        ...
        # X_test_round = test_data_round[features]
        ...
        # X_test_round = scaler.transform(X_test_round)
        ...
        # X_test_round = Variable(torch.from_numpy(X_test_round).float()).to(device)
        ...
        for epoch in range(1001):
            ...
            # pred_test_round = convert_data(X_test_round)
            ...
            if epoch <0:
                ...
                loss_sum = loss+Min_t(pred_est, 0)+(1/1000)*loss_curie+(1/1000)*Max_t_580(pred_curie_580ika)+(1/1000)*Min_t_580(pred_curie_580izyou)#+Max_t(pred_curie_580izyou,800)
            else:
                pc_est = torch.cat((est_data_origin, pred_est), 1)
                # pc_test = torch.cat((test_data_round_origin,pred_test_round),1)
                
                # sv_value = epoch %2
                # sv_xy_origin = calc_pred_semivariogram_xy(pc_est,dist_xy_list,sv_value)
                # sv_xy = sv_xy_origin[~(sv_xy_origin.isnan() | model_sv_xy_origin[:,[sv_value,sv_value+2]].isnan())]
                # model_sv_xy = model_sv_xy_origin[:,[sv_value,sv_value+2]][~(sv_xy_origin.isnan() | model_sv_xy_origin[:,[sv_value,sv_value+2]].isnan())]
                # loss_sv_xy = F.mse_loss(sv_xy,model_sv_xy)
                # loss_sv_xy = torch.sqrt(loss_sv_xy)
                
                #feature
                sv_f = calc_pred_feature_semivariogram(pc_est,dist_f_list)
                loss_sv_f = F.mse_loss(sv_f[~model_sv_f.isnan()],model_sv_f[~model_sv_f.isnan()])
                loss_sv_f = torch.sqrt(loss_sv_f)
                
                # loss_sv_xy = torch.tensor(0)
                
                sv_z = calc_pred_semivariogram_z(pc_est,est_data_origin_xy,dist_z_list)
                # sv_z_test = calc_pred_semivariogram_z_2(pc_test,test_data_round_origin_xy,dist_z_list_test)
                # sv_z = torch.cat((sv_z,sv_z_test),0)
                loss_sv_z=F.mse_loss(sv_z[~(model_sv_z.isnan()|sv_z.isnan())], model_sv_z[~(model_sv_z.isnan()|sv_z.isnan())])
                loss_sv_z=torch.sqrt(loss_sv_z)
                
                # loss_sv_z_test=F.mse_loss(sv_z_test[~(model_sv_z_test.isnan()|sv_z_test.isnan())], model_sv_z_test[~(model_sv_z_test.isnan()|sv_z_test.isnan())])
                # loss_sv_z_test=torch.sqrt(loss_sv_z_test)
                
                # if epoch%1 ==0:
                #     print(epoch,loss_v.data.item(),loss_sv_f.data.item(),loss_sv_z.data.item(),f"time:{(time.time()-time_start)/60}")
                
                nk_hyper_f, nk_hyper_z = 10,500
                loss_sv = (1/nk_hyper_f)*loss_sv_f + (1/nk_hyper_z)*loss_sv_z
                loss_sum = loss+Min_t(pred_est, 0)+(1/1000)*loss_curie+(1/1000)*Max_t_580(pred_curie_580ika)+(1/1000)*Min_t_580(pred_curie_580izyou)+loss_sv #Max_t(pred_curie_580izyou,800)+
            ...
            if epoch % 10 == 0:
                print(
                    f'{epoch},{np.sqrt(loss.data.item())},{np.sqrt(loss_v.data.item())},{np.sqrt(loss_curie.data.item())},{np.sqrt(loss_sv.data.item())},{np.sqrt(loss_sv_f.data.item())},{np.sqrt(loss_sv_z.data.item())},time:{(time.time()-time_start)/60}')
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss','sv_loss']).to_csv(
            f'./output_japan_last/learning_curve/nk/lc_nk_svf_{nk_hyper_f}_{nk_hyper_z}_inter_{name}_{num}.csv', index=False)
        print('-'*50)
        est_data['t'] = convert_data(X_est).detach().cpu().numpy()
        est_data = est_data[['x', 'y', 'h_z', 't']]
        est_data.to_csv(f'./output_japan_last/voxler/est_nk_svf_{nk_hyper_f}_{nk_hyper_z}_inter_{name}_{num}.csv', index=False)
        torch.save(model.state_dict(), f'./output_japan_last/model/torch_nk_svf_{nk_hyper_f}_{nk_hyper_z}_inter_{name}_{num}.pth')
...
bot.send(f"inter{args.number}:RMSE {np.sqrt(loss_v.data.item())}")

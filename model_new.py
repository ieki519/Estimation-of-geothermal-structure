...
for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
    ...
    depth_features = ['x', 'y', 'h', 'z', 'h_z', 'volcano', 'curie', 'age_a', 'age_b',
                     'group_freq', 'Temp', 'pH', 'Na', 'K',
                    'Ca', 'Mg', 'Cl', 'SO4', 'HCO3', 'anion', 'kmeans', 'age']
    ...
    grad_features = ['x', 'y', 'h', 'z', 'h_z', 'volcano', 'curie', 'age_a', 'age_b',
                     'group_freq', 'Temp', 'pH', 'Na', 'K',
                    'Ca', 'Mg', 'Cl', 'SO4', 'HCO3', 'anion', 'kmeans', 'age', 'depth0',
       'depth500', 'depth1000']
    ...
    features = ['x', 'y', 'h', 'z', 'h_z', 'volcano', 'curie', 'age_a', 'age_b',
                     'group_freq',  'Temp', 'pH', 'Na', 'K',
                    'Ca', 'Mg', 'Cl', 'SO4', 'HCO3', 'anion', 'kmeans', 'age','depth0',
       'depth500', 'depth1000','grad','grad_max','grad_min']
    target = 't'
    ...
    pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
        f'./output/learning_curve/lc_dnn_inter_depthあり_{num}.csv', index=False)
    print('-'*50)
pred_est = np.hstack(pred_est_list).mean(axis=1)
est_data['t'] = pred_est
est_data = est_data[['x', 'y', 'h_z', 't']]
est_data.to_csv(f'./output/voxler/est_dnn_inter_depthあり.csv', index=False)
...
pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output/learning_curve/lc_dnn_inter_depthあり.csv', index=False)
#%%
...
for num in range(5):
    ...
    grad_features = ['x', 'y', 'h', 'z', 'h_z', 'volcano', 'curie', 'age_a', 'age_b',
                     'group_freq',  'Temp', 'pH', 'Na', 'K',
                    'Ca', 'Mg', 'Cl', 'SO4', 'HCO3', 'anion', 'kmeans', 'age','depth0','depth500']
    grad_target_list = ['grad','grad_max','grad_min']
    ...
    features = ['x', 'y', 'h', 'z', 'h_z', 'volcano', 'curie', 'age_a', 'age_b',
                     'group_freq',  'Temp', 'pH', 'Na', 'K',
                    'Ca', 'Mg', 'Cl', 'SO4', 'HCO3', 'anion', 'kmeans', 'age','depth0','depth500','grad','grad_max','grad_min']
    target = 't'
    ...
    learning_curve_list_master.append(np.array(learning_curve_list))
    pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
        f'./output/learning_curve/lc_dnn_extra_depthあり_{num}.csv', index=False)
    print('-'*50)
pred_est = np.hstack(pred_est_list).mean(axis=1)
est_data['t'] = pred_est
est_data = est_data[['x', 'y', 'h_z', 't']]
est_data.to_csv(f'./output/voxler/est_dnn_extra_depthあり.csv', index=False)
learning_curve=np.vstack(learning_curve_list_master).mean(axis=0)
learning_curve=np.vstack([np.arange(0,2001,100),learning_curve])
pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output/learning_curve/lc_dnn_extra_depthあり.csv', index=False)
#%%

#%%
learning_curve_list_master[0]
#%%
# train_loss_list=[]
# test_loss_list=[]
# for i in range(10):

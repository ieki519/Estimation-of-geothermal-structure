...
features_name_list = ["volcano","curie","onsen","tishitsu_rank","tishitsu_ohe","depth","grad"]
...
for i in range(2**N):
    ...
    if "tishitsu_rank" in A and "tishitsu_ohe" in A:
        continue
...
for all_f in [["curie","onsen","tishitsu_ohe","depth","grad"]]:
    ...
    for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
        if num == args.number:
            ...
            test_data = test_data[test_data.h_z==-500]
            pred_t = test_data.pred_t.values
            measure_t = test_data.measure_t.values
            print(np.mean(np.abs(pred_t-measure_t)/measure_t)*100)
            print(np.sqrt(np.mean((pred_t-measure_t)**2)))
            print(np.mean(measure_t),np.std(measure_t))
            print(measure_t.shape)
            #-----------------------------------
            
            # pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
            #     f'./output_japan_last/learning_curve/select_features/lc_dnn_inter_{name}_{num}.csv', index=False)
            ...
            # est_data.to_csv(f'./output_japan_last/voxler/est_dnn_inter_{name}_{num}.csv', index=False)
            print('-'*50)
    # pred_est = np.hstack(pred_est_list).mean(axis=1)
    # est_data['t'] = pred_est
    # est_data = est_data[['x', 'y', 'h_z', 't']]
    # est_data.to_csv(f'./output_japan/voxler/est_dnn_inter_{name}.csv', index=False)
    ...
    # pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output_japan/learning_curve/select_features/lc_dnn_inter_{name}.csv', index=False)

...
for num in range(5):
    if num==args.number:
        ...
        # grad_feature
        if "grad" in name:
            add_input_grad = pd.read_csv(f'./input_japan/grad/add_input_grad_ja.csv')#80
            add_grid_grad = pd.read_csv(f'./input_japan/grad/add_grid_grad_detail_ja.csv')#80
        ...
        for sv_target in sv_target_list:
            ...
            if sv_target=="sv_250" or sv_target=="sv_1000":
            ...
            elif sv_target=="sv_500":
                xgb=XGBRegressor(random_state=0)
                xgb.fit(X_train[sv_features],X_train[sv_target])
                train_data_plane_isnull[sv_target] = xgb.predict(X_train_isnull)
                test_data_plane[sv_target] = xgb.predict(X_test)
                est_data_plane[sv_target] = xgb.predict(X_est)
            elif sv_target=="sv_750":
                rf=RandomForestRegressor(random_state=0)
                rf.fit(X_train[sv_features],X_train[sv_target])
                train_data_plane_isnull[sv_target] = rf.predict(X_train_isnull)
                test_data_plane[sv_target] = rf.predict(X_test)
        ...
        #     f'./output_japan_last/learning_curve/nk/lc_nk{nk_hyper}_extra_{name}_{num}.csv', index=False)
        print('-'*50)
        est_data['t'] = convert_data(X_est).detach().cpu().numpy()
        est_data = est_data[['x', 'y', 'h_z', 't']]
        est_data.to_csv(f'./output_japan_last/voxler/est_nk{nk_hyper}_nonave_{name}_{num}.csv', index=False)
        torch.save(model.state_dict(), f'./output_japan_last/model/torch_nk{nk_hyper}_nonave_{name}_{num}.pth')

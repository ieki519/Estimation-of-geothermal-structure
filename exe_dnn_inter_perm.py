    ...
    for except_f in features_dict[all_f]:
        ...
        for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
            ...
            pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
                f'./output_japan/learning_curve/perm/lc_dnn_inter_{name}_{except_name}_{num}.csv', index=False)
            torch.save(model.state_dict(), f'./output_japan/model/torch_dnn_inter_{name}_{except_name}_{num}.pth')
            print('-'*50)
        pred_est = np.hstack(pred_est_list).mean(axis=1)
        est_data['t'] = pred_est
        est_data = est_data[['x', 'y', 'h_z', 't']]
        est_data.to_csv(f'./output_japan/voxler/est_dnn_inter_{name}_{except_name}.csv', index=False)
        ...
        pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output_japan/learning_curve/perm/lc_dnn_inter_{name}_{except_name}.csv', index=False)

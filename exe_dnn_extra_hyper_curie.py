...
for curie_num in [100000,1000000]:
    ...
    for num in range(5):
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
            f'./output_japan_last/learning_curve/curie_hyper/lc_dnn_extra_{name}_{num}.csv', index=False)
    ...
    est_data.to_csv(f'./output_japan_last/voxler/est_dnn_extra_{name}.csv', index=False)
    ...
    pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output_japan_last/learning_curve/curie_hyper/lc_dnn_extra_{name}.csv', index=False)

...
features_name_list = ["volcano","curie","onsen","tishitsu_ohe","depth","grad"]
...
for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
    ...
    if "tishitsu_rank" in name:
        ...
        est_data_detail["group_rank"]=est_data_detail["group_ja"].replace(tishitsu_rank_dict)
    ...
    if "depth" in name:
        ...
        est_data_input_xy = est_data_input_xy.merge(add_input_depth, how='left', on=['x', 'y'])
    ...
    if "grad" in name:
        ...
        est_data_input_xy = est_data_input_xy.merge(add_input_grad, how='left', on=['x', 'y'])
    ...
    model.load_state_dict(torch.load(f'./output_japan_last/model/torch_nk500_inter_{name}_{num}.pth'))
...
#     pred_est_list.append(convert_data(X_est_input_xy).detach().cpu().numpy())
# est_data_input_xy['t'] = np.hstack(pred_est_list).mean(axis=1)
# est_data_input_xy = est_data_input_xy[['x', 'y', 'h_z', 't']]
# est_data_input_xy.to_csv(f"./output_japan_last/voxler/nk/est_nk500_output_{name}_detail_input_xy.csv", index=False)
    
    #-----------------------------------
    test_data["pred_t"] = convert_data(X_test).detach().cpu().numpy()
    test_data["measure_t"]=Y_test.detach().cpu().numpy()
    test_data["h_z"]=test_data["h_z"].round(-1)
    test_data=test_data.groupby(["x","y","h_z"],as_index=False).mean()
    test_data = test_data[test_data.h_z==-1000]
    pred_t = test_data.pred_t.values
    measure_t = test_data.measure_t.values
    mape_list.append(np.mean(np.abs(pred_t-measure_t)/measure_t)*100)
    rmse_list.append(np.sqrt(np.mean((pred_t-measure_t)**2)))
    
print(np.mean(mape_list))
print(np.mean(rmse_list))
#-----------------------------------
    
#     pred_est_list.append(convert_data(X_est_detail).detach().cpu().numpy())
# est_data_detail['t'] = np.hstack(pred_est_list).mean(axis=1)
# est_data_detail = est_data_detail[['x', 'y', 'h_z', 't']]
# est_data_detail.to_csv(f'./output_japan_last/voxler/nk/est_nk_output_{name}_detail.csv', index=False)

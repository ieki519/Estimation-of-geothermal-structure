    ...
    if num==args.number:
        ...
        for epoch in range(1001):
            ...
            if epoch <0:
                ...
                # if epoch%10 ==0:
                #     print(epoch,loss_v.data.item(),loss_sv_xy.data.item(),loss_sv_z.data.item(),f"time:{(time.time()-time_start)/60}")
                loss_sv = loss_sv_xy + loss_sv_z
                
                nk_hyper = 1
                loss_sum = loss+Min_t(pred_est, 0)+(1/1000)*loss_curie+(1/1000)*Max_t_580(pred_curie_580ika)+(1/1000)*Min_t_580(pred_curie_580izyou)+(1/nk_hyper)*loss_sv #Max_t(pred_curie_580izyou,800)+
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss','sv_loss']).to_csv(
            f'./output_japan_last/learning_curve/nk/lc_nk{nk_hyper}_inter_{name}_{num}.csv', index=False)
        print('-'*50)
        est_data['t'] = convert_data(X_est).detach().cpu().numpy()
        est_data = est_data[['x', 'y', 'h_z', 't']]
        est_data.to_csv(f'./output_japan_last/voxler/est_nk{nk_hyper}_inter_{name}_{num}.csv', index=False)
        torch.save(model.state_dict(), f'./output_japan_last/model/torch_nk{nk_hyper}_inter_{name}_{num}.pth')

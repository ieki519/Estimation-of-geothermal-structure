...
for all_f in all_features_name_list:
    ...
    for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
            f'./output_japan/learning_curve/features/lc_dnn_inter_{name}_{num}.csv', index=False)
        
        torch.save(model.state_dict(), f'./output_japan/model/torch_dnn_inter_{name}_{num}.pth')
        
        est_data['t'] = convert_data(X_est).detach().cpu().numpy()
        est_data = est_data[['x', 'y', 'h_z', 't']]
        est_data.to_csv(f'./output_japan/voxler/est_dnn_inter_{name}_{num}.csv', index=False)
        print('-'*50)
    ...
    pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output_japan/learning_curve/features/lc_dnn_inter_{name}.csv', index=False)

...
#%%
seed=4
for name in master_features_dict.keys():
    ...
    for num, (idx_trn, idx_tst) in enumerate(cv.split(xy_data)):
        fix_seed(seed)
        print(seed)
        master_features=master_features_dict[name].copy()
        # preprocess
        ...
        pd.DataFrame(np.array(loss_list), columns=['epoch', 'train_loss', 'test_loss', 'curie_loss']).to_csv(
            f'./output/learning_curve/lc_dnn_inter_{name}_seed{seed}_{num}.csv', index=False)
        torch.save(model.state_dict(), f'./output/model/torch_dnn_inter_{name}_seed{seed}_{num}.pth')
        print('-'*50)
    pred_est = np.hstack(pred_est_list).mean(axis=1)
    est_data['t'] = pred_est
    est_data = est_data[['x', 'y', 'h_z', 't']]
    est_data.to_csv(f'./output/voxler/est_dnn_inter_{name}_seed{seed}.csv', index=False)
    ...
    pd.DataFrame(learning_curve.T,columns=['epoch', 'test_loss']).to_csv(f'./output/learning_curve/lc_dnn_inter_{name}_seed{seed}.csv', index=False)
